...
if(window.isAndroid){window.minScrollToSlide = 50;}
if(window.isSafari && window.isMobile){window.minScrollToSlide = 70;}
//window.minSwipeToSlide = 4; // not used anymore since "touchSwipe" plugin was removed from slides.js && plugins.js
window.awaitToSlideDown = 300; // timeout to prevent the immediate change of a slide with a scrollable content in it when the end of the content is reached
//window.enableMobileZoom = 0; // not used
//window.frameWidthToDetectZoom = 1234; // pass the width of a frame as this var if you include this page into another one as an iframe. Do that from the parent page like this: $("#iframe").get(0).contentWindow.frameWidthToDetectZoom = $("#iframe").width();
...
function updateHash(){
  ...
  if (hashLink) {
    ...
    if(hashLink.indexOf("slidesScrollDebug")!=-1){
