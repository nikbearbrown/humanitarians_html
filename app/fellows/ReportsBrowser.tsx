...
export default function ReportsBrowser({ groups }: { groups: Group[] }) {
  ...
  return (
    <>
      ...
      {totalDocs === 0 ? (
        <p className="text-muted-foreground">{query || activeTag ? 'No reports match your search.' : 'No reports yet.'}</p>
      ) : (
          ...
          {filteredGroups.map(g => (
            <section key={g.folder}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {g.docs.map(doc => (
                  <Link key={doc.slug} href={`/reports/${doc.slug}`}>
