...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (published && !current.published_at) publishedAt = body.published_at || new Date().toISOString()
    const rows = await sql`UPDATE blog_posts SET title = ${title}, subtitle = ${subtitle}, slug = ${slug}, byline = ${byline}, cover_image = ${cover_image}, content = ${content}, excerpt = ${excerpt}, published = ${published}, tags = ${tags}, published_at = ${publishedAt}, updated_at = NOW() WHERE id = ${id} RETURNING *`
    if (rows.length === 0) return NextResponse.json({ error: 'Not found' }, { status: 404 })
    return NextResponse.json(rows[0])
  } catch (error: unknown) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Database error' }, { status: 500 }) }
}
