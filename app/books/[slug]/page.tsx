...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!book) return { title: 'Books - Humanitarians AI' }
    ...
    <div className="container px-4 md:px-6 mx-auto py-12">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-10">
          {book.cover && (
            <div className="shrink-0"><img src={book.cover} alt={`${book.title} cover`} className="w-48 rounded-md border shadow-sm" /></div>
          )}
          <div className="flex-1">
            ...
            {book.keywords.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-6">
                {book.keywords.map(k => (<Badge key={k} variant="secondary" className="text-xs">{k}</Badge>))}
              </div>
            )}
          </div>
        </div>
