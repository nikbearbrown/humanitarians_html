                  ...
                  <Link key={doc.slug} href={`/dev/${doc.slug}`}>
                    <Card className="h-full hover:border-foreground/20 transition-colors cursor-pointer">
                      <CardHeader>
                        ...
                        {doc.tags.length > 0 && (<div className="flex flex-wrap gap-1 pt-2">{doc.tags.slice(0, 3).map(tag => (<Badge key={tag} variant="secondary" className="text-[10px]">{tag}</Badge>))}{doc.tags.length > 3 && <span className="text-[10px] text-muted-foreground">+{doc.tags.length - 3} more</span>}</div>)}
                      </CardHeader>
                    </Card>
                  </Link>
                ))}
