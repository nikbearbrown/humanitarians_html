...
export default async function SectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  const articles = await sql`SELECT * FROM substack_articles WHERE section_id = ${sectionData.id} ORDER BY published_at DESC`
            ...
            <div className="divide-y">
              {articles.map((a: Record<string, string>) => (
                <article key={a.id} className="py-6 first:pt-0">
                  <Link href={`/substack/${section}/${a.slug}`} className="group block">
                    {a.display_date && <time className="text-sm text-muted-foreground">{a.display_date}</time>}
                    <h2 className="text-xl font-semibold mt-1 group-hover:underline">{a.title}</h2>
                    {a.subtitle && <p className="text-muted-foreground mt-1">{a.subtitle}</p>}
                    {a.excerpt && <p className="text-sm text-muted-foreground mt-2">{a.excerpt}</p>}
                  </Link>
                </article>
              ))}
            </div>
