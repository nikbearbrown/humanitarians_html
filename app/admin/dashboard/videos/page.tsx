  ...
  async function runImport() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/videos/import-youtube', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ input: importInput, tags, autoTag: importAutoTag || undefined }),
      })
      const data = await res.json()
    ...
    <div className="space-y-6">
      ...
      {allTags.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button onClick={() => setFilterTag(null)} className={`px-2.5 py-1 text-xs rounded-full border transition-colors ${!filterTag ? 'bg-black text-white dark:bg-white dark:text-black' : 'border-input hover:bg-accent'}`}>
            All ({videos.length})
          </button>
          {allTags.sort().map(tag => {
            const count = videos.filter(v => v.tags?.includes(tag)).length
            return (
              <button key={tag} onClick={() => setFilterTag(filterTag === tag ? null : tag)} className={`px-2.5 py-1 text-xs rounded-full border transition-colors ${filterTag === tag ? 'bg-black text-white dark:bg-white dark:text-black' : 'border-input hover:bg-accent'}`}>
                {tag} ({count})
              </button>
            )
          })}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Button variant="outline" size="sm" onClick={() => { setSelectMode(!selectMode); selectNone() }} className="gap-2">
            ...
            {selectedIds.size > 0 && (
              <>
                <Button size="sm" onClick={() => bulkUpdatePublished(true, 'selected')} className="gap-2">
                  <Eye className="h-3.5 w-3.5" /> Publish {selectedIds.size} Selected
                </Button>
                <Button variant="outline" size="sm" onClick={() => bulkUpdatePublished(false, 'selected')} className="gap-2">
                  <EyeOff className="h-3.5 w-3.5" /> Unpublish {selectedIds.size} Selected
                </Button>
        ...
        {filterTag && (
          <>
            <Button size="sm" onClick={() => bulkUpdatePublished(true, 'tag')} className="gap-2">
              <Eye className="h-3.5 w-3.5" /> Publish All &ldquo;{filterTag}&rdquo;
            </Button>
            <Button variant="outline" size="sm" onClick={() => bulkUpdatePublished(false, 'tag')} className="gap-2">
              <EyeOff className="h-3.5 w-3.5" /> Unpublish All &ldquo;{filterTag}&rdquo;
            </Button>
            <Button variant="destructive" size="sm" onClick={() => bulkDeleteByTag(filterTag)} className="gap-2">
              <Trash2 className="h-3.5 w-3.5" /> Delete All &ldquo;{filterTag}&rdquo;
            </Button>
          </>
        )}
      </div>
                ...
                <div className="flex items-start gap-3">
                  {selectMode && (
                    <button onClick={() => toggleSelect(v.id)} className="mt-1">
                      {selectedIds.has(v.id) ? <CheckSquare className="h-5 w-5 text-primary" /> : <Square className="h-5 w-5 text-muted-foreground" />}
                    </button>
                  )}
                  <div className="space-y-1">
                    <CardTitle className="text-lg flex items-center gap-2 flex-wrap">
                      {v.title}
                      {v.pinned && <Badge variant="default" className="gap-1"><Pin className="h-3 w-3" /> Pinned</Badge>}
                      <Badge variant={v.published ? 'secondary' : 'outline'}>{v.published ? 'Published' : 'Draft'}</Badge>
                    </CardTitle>
                    <CardDescription className="flex items-center gap-2 flex-wrap">
                      <Badge variant="outline">{v.youtube_id}</Badge>
                      {v.tags?.map(tag => <Badge key={tag} variant="secondary" className="text-xs">{tag}</Badge>)}
                    </CardDescription>
                    {v.description && <p className="text-sm text-muted-foreground pt-1 line-clamp-2">{v.description}</p>}
                  </div>
                </div>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => openEdit(v)}><Pencil className="h-3.5 w-3.5" /></Button>
                  <Button variant="outline" size="sm" onClick={() => deleteVideo(v.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </CardHeader>
              <CardContent>
                <div className="aspect-video rounded-md overflow-hidden max-w-sm">
                  <iframe src={`https://www.youtube.com/embed/${v.youtube_id}`} title={v.title} width="100%" height="100%" frameBorder="0" allowFullScreen loading="lazy" className="w-full h-full" />
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{editingVideo ? 'Update the video details.' : 'Add a new video to the directory.'}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="video-title">Title</Label>
              <Input id="video-title" value={form.title} onChange={e => setForm(prev => ({ ...prev, title: e.target.value }))} placeholder="e.g. Introduction to AI for Good" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="video-youtube-id">YouTube Video ID</Label>
              <Input id="video-youtube-id" value={form.youtube_id} onChange={e => setForm(prev => ({ ...prev, youtube_id: e.target.value }))} placeholder="e.g. hWgWmLWivpo" />
              <p className="text-xs text-muted-foreground">The ID from the YouTube URL (e.g. youtube.com/watch?v=<strong>hWgWmLWivpo</strong>)</p>
            </div>
            <div className="space-y-2">
              <Label htmlFor="video-desc">Description</Label>
              <Textarea id="video-desc" value={form.description} onChange={e => setForm(prev => ({ ...prev, description: e.target.value }))} placeholder="Brief description of the video" rows={3} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="video-tags">Tags (comma-separated)</Label>
              <Input id="video-tags" value={form.tags_input} onChange={e => setForm(prev => ({ ...prev, tags_input: e.target.value }))} placeholder="e.g. ai-for-good, lecture" />
            </div>
            <div className="flex items-center gap-6">
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={form.pinned} onChange={e => setForm(prev => ({ ...prev, pinned: e.target.checked }))} className="rounded" />
                Pinned (always shown at top)
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input type="checkbox" checked={form.published} onChange={e => setForm(prev => ({ ...prev, published: e.target.checked }))} className="rounded" />
                Published
              </label>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={saveVideo} disabled={saving}>{saving ? 'Saving...' : 'Save'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Import videos from a YouTube channel or playlist as drafts. Duplicates are skipped automatically.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="import-input">Channel or Playlist URL</Label>
              <Input id="import-input" value={importInput} onChange={e => setImportInput(e.target.value)} placeholder="e.g. @YourChannel, youtube.com/playlist?list=PLxxx, or channel URL" />
              <p className="text-xs text-muted-foreground">Accepts: @handle, channel URL, playlist URL, or raw IDs</p>
            </div>
            <div className="space-y-2">
              <Label htmlFor="import-tags">Tags to apply (comma-separated, optional)</Label>
              <Input id="import-tags" value={importTags} onChange={e => setImportTags(e.target.value)} placeholder="e.g. ai-for-good, lecture" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="import-autotag">Auto-tag name (optional)</Label>
              <Input id="import-autotag" value={importAutoTag} onChange={e => setImportAutoTag(e.target.value)} placeholder="e.g. playlist name — added as a tag to all imported videos" />
            </div>
            {importResult && (
          ...
          <DialogFooter>
            ...
            <Button onClick={runImport} disabled={importing || !importInput.trim()}>{importing ? 'Importing...' : 'Import as Drafts'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={playlistsOpen} onOpenChange={setPlaylistsOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Enter a channel URL or @handle to see all playlists. Click one to import it.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="flex gap-2">
              <Input value={playlistInput} onChange={e => setPlaylistInput(e.target.value)} placeholder="e.g. @YourChannel or channel URL" className="flex-1" />
              <Button onClick={browsePlaylists} disabled={loadingPlaylists || !playlistInput.trim()}>{loadingPlaylists ? 'Loading...' : 'Browse'}</Button>
            </div>
            {playlists.length > 0 && (
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {playlists.map(pl => (
                  <button key={pl.id} onClick={() => importPlaylist(pl)} className="w-full text-left p-3 rounded-md border hover:bg-accent transition-colors">
                    <div className="font-medium text-sm">{pl.title}</div>
                    <div className="text-xs text-muted-foreground">{pl.videoCount} video{pl.videoCount !== 1 ? 's' : ''} &middot; {pl.id}</div>
                  </button>
                ))}
              </div>
            )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPlaylistsOpen(false)}>Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
